...
> **Intra-run dedup (finding #7).** This run did not apply intra-survivor dedup at certification time. Re-measured with `crm.novelty.dedup_survivors` (same MiniLM embedder, delta=0.35), the **7** certified survivors collapse to **4** distinct clusters (3 are intra-run near-duplicates) — see [`docs/findings/dedup_collapse.md`](docs/findings/dedup_collapse.md). The updated `certify_novel` gate now blocks such duplicates at admission.
